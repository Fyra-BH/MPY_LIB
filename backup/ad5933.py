...
class AD5933:
    def __init__(self, i2c):
         ...
         try:
             ...
             args = ujson.load(f)
             self.GAIN_FACTOR, self.CLK_FREQ, self.SYS_PHASE, self.start_f, self.step, self.num, self.cycle = \
             args["gain_factor"], args["clk_freq"], args["sys_phase"], args["start_f"], args["step"], args["num"],args["cycle"] \
...
#         print('write ' + str(bin(cmd)))
        if self.PGA == 1: cmd = cmd | 0x01
        self.write_reg_int(REG_CTRL1, cmd)
    #选择时钟源, c = 0内部时钟, c != 0外部时钟
    def select_clk(self, c):
        if c == 0: self.write_reg_int(REG_CTRL2, 0x00)
        else: self.write_reg_int(REG_CTRL2,  0xff)
    #获取状态
    def get_sta(self):
    ...
    def get_temp(self):
        ...
        while(self.get_sta()&STA_TEMP_CONVERT_EFECT != 1): pass
        temp = 0
        temp = self.read_reg(REG_TEMP1)*256
        temp += self.read_reg(REG_TEMP2)
        if temp&0x2000 == 1: return (temp - 16384)/32 #负温度
        else: return temp/32 #正温度
    #设置起始频率
    ...
    def set_scycle(self, num, times = 1):
        ...
        reg_val = (num>>8)&0x01
        if times == 2: reg_val = reg_val | (0x01 << 1)
        elif times == 4: reg_val = reg_val | (0x3 << 1)
    ...
    def read_ri_data(self):#read real and imagine data
        ...
        reg_imag = self.read_reg(REG_IMAG_DATA1) << 8
        reg_imag += self.read_reg(REG_IMAG_DATA2)
        reg_real, reg_imag = _s_int16(reg_real), _s_int16(reg_imag)
        return reg_real, reg_imag
    #等待数据转换
    def wait_data_convert(self):
        while(self.get_sta()&STA_MEASURE_EFECT != 0x02): pass
    #扫描是否结束
    def scan_fin(self):
        return self.get_sta()&STA_FREQ_SCAN_FIN == 0x04
    def get_phase(self, re, im):
        phase = 0.0
        if re > 0 and im > 0: phase = math.atan(im / re) * 180 / math.pi
        if re < 0 and im > 0: phase = math.atan(im / re) * 180 / math.pi + 180
        if re < 0 and im < 0: phase = math.atan(im / re) * 180 / math.pi + 180
        if re > 0 and im < 0: phase = math.atan(im / re) * 180 / math.pi
        return phase
    ...
    def set_up(self, ref_r = 10000, clk_freq = 16776000, start_freq = 10000, freq_step = 500, num_inc_freq = 180, cycle = 15):
        ...
        reg_real, reg_imag = self.read_ri_data()
        print("reg_real = " + str(reg_real))
        print("reg_imag = " + str(reg_imag))
        temp = (reg_real**2 + reg_imag**2)**0.5
        self.GAIN_FACTOR = (1 / ref_r) / temp
        self.SYS_PHASE = array.array("f",[])
        while(not self.scan_fin()):
            self.wait_data_convert()
            reg_real, reg_imag = self.read_ri_data()
            self.SYS_PHASE.append(self.get_phase(reg_real, reg_imag))
        ...
        args["gain_factor"], args["clk_freq"], args["start_f"], args["step"], args["num"],args["cycle"], args["sys_phase"], \
         = self.GAIN_FACTOR, self.CLK_FREQ, self.start_f, self.step, self.num, self.cycle, list(self.SYS_PHASE)
        ujson.dump(args, f)
        f.close()
    ...
    def measure(self):
        ...
        reg_real, reg_imag = self.read_ri_data()
        temp = (reg_real**2 + reg_imag**2)**0.5
        return (1/self.GAIN_FACTOR/temp)
    
    def scan_freq(self):
        print("start =", self.start_f, ", end =", self.start_f + self.step * self.num, ", step =", self.step)
        ...
        while(not self.scan_fin()):
            ...
            reg_real, reg_imag = self.read_ri_data()
            temp = (reg_real**2 + reg_imag**2)**0.5
            z = (1/self.GAIN_FACTOR/temp)
            res_z.append(z)
            res_p.append(self.get_phase(reg_real, reg_imag) - self.SYS_PHASE[cnt])
            self.write_cmd(CMD_STEPUP_FREQ)
            cnt += 1
        return res_z, res_p
